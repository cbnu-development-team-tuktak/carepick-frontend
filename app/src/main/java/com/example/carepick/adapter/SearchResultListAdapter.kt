...
import com.example.carepick.viewHolder.DoctorSearchListViewHolder
...
class SearchResultListAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        return when (viewType) {
            ...
            TYPE_HOSPITAL -> {
                ...
                // 병원 뷰홀더를 호출한다
                HospitalSearchListViewHolder(binding, activity)
            }
            // 뷰 타입이 의사인 경우
            TYPE_DOCTOR -> {
                // search_list.xml 레이아웃을 바인딩한다
                val binding = SearchListBinding.inflate(LayoutInflater.from(parent.context), parent, false)
                // 의사 뷰홀더를 호출한다
                DoctorSearchListViewHolder(binding, activity)
            }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (val item = items[position]) {
            is HospitalDetailsResponse -> (holder as HospitalSearchListViewHolder).bind(item)
            is DoctorDetailsResponse -> (holder as DoctorSearchListViewHolder).bind(item)
        }
    }
}